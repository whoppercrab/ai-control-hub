...
const trendingModels = [
  { id: 1, author: "kykim", name: "bert-base-korean-v1", task: "Fill-Mask", likes: 342, downloads: "1.2M", updated: "2 days ago", type: "nlp" },
  { id: 2, author: "stabilityai", name: "stable-diffusion-xl-base-1.0", task: "Text-to-Image", likes: 15400, downloads: "50M", updated: "10 hours ago", type: "vision" },
  { id: 3, author: "meta-llama", name: "Llama-3-70b-chat-hf", task: "Text Generation", likes: 8900, downloads: "5.4M", updated: "Just now", type: "nlp" },
  { id: 4, author: "google", name: "gemma-7b-it", task: "Text Generation", likes: 6700, downloads: "3.1M", updated: "5 days ago", type: "nlp" },
  { id: 5, author: "openai-community", name: "gpt2-large", task: "Text Generation", likes: 4500, downloads: "22M", updated: "3 years ago", type: "nlp" },
  { id: 6, author: "beomi", name: "KoAlpaca-Polyglot-12.8B", task: "Text Generation", likes: 1200, downloads: "50k", updated: "1 month ago", type: "nlp" },
];
...
const trendingDatasets = [
  ...
  { id: 104, author: "common_voice", name: "common_voice_13_0", size: "80GB", likes: 3200, downloads: "800k", updated: "5 days ago", type: "audio" },
  { id: 105, author: "klue", name: "klue-benchmark", size: "45MB", likes: 1500, downloads: "300k", updated: "8 months ago", type: "nlp" },
  { id: 106, author: "ai-hub", name: "korean-dialogue-summary", size: "2.1GB", likes: 900, downloads: "100k", updated: "1 month ago", type: "nlp" },
];
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="relative max-w-2xl mx-auto group">
            ...
            <div className="absolute inset-y-0 right-3 flex items-center">
                <button className="bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-full text-sm font-medium transition-colors border border-gray-200">⌘ K</button>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 border-b border-gray-200 pb-2">
            <div className="flex gap-6">
                <button onClick={() => setViewType('models')} className={`flex items-center gap-2 pb-2 text-lg font-bold border-b-2 transition-all ${viewType === 'models' ? "border-blue-600 text-gray-900" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
                    <Box size={20} /> Models <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full text-gray-500 font-normal">34k</span>
                </button>
                <button onClick={() => setViewType('datasets')} className={`flex items-center gap-2 pb-2 text-lg font-bold border-b-2 transition-all ${viewType === 'datasets' ? "border-red-500 text-gray-900" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
                    <Database size={20} /> Datasets <span className="text-xs bg-gray-100 px-2 py-0.5 rounded-full text-gray-500 font-normal">8k</span>
                </button>
            </div>
            <div className="flex items-center gap-3 overflow-x-auto">
                ...
                <button className="flex items-center gap-2 text-sm font-medium text-gray-500 border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50"><SlidersHorizontal size={14}/> Filter</button>
            </div>
        </div>

        {/* 그리드 리스트 */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
            {viewType === 'models' && trendingModels.map((model) => (
                <Link href="/model" key={model.id} className="group block">
                    <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all hover:border-blue-300 h-full flex flex-col">
                        ...
                        <div className="mb-6 flex-1"><span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-600 border border-gray-200"><Terminal size={12}/> {model.task}</span></div>
                        <div className="flex items-center justify-between text-xs text-gray-500 pt-4 border-t border-gray-50">
                            <div className="flex gap-4"><span className="flex items-center gap-1"><Activity size={12}/> {model.updated}</span><span className="flex items-center gap-1"><Download size={12}/> {model.downloads}</span></div>
                            <span className="flex items-center gap-1 font-medium hover:text-red-500"><Heart size={12}/> {model.likes}</span>
                        </div>
                    </div>
                </Link>
            ))}
